...
- **Casing** — Headlines in sentence case, occasionally with a highlighted noun in accent blue (*"built for **resellers**"*). Buttons in Title Case for primary actions ("Get API keys", "Start building", "Read the docs"). Eyebrows ALL CAPS with wide tracking (`0.16em`).
...
### Color
- **One action color: bright blue `#1E66FF`.** Every primary button, link, focus ring, active tab underline, brand mark, and highlighted headline word. Hover darkens to `#0F52E0` (accent-600), press to `#0A3FB0` (accent-700). The system is deliberately monochromatic on action — no rainbow.
- **Deep slate "ink" `#0C1322`** anchors dark surfaces: nav-on-scroll glassy white aside, dark CTA sections, the onboarding brand rail, dashboard hero cards.
- **Code panels are near-black `#0B1020`** with a slightly lifted header `#121A2C`.
- **Backgrounds are overwhelmingly white** (`#FFFFFF`), with `#F7F8FA` for subtle insets and a whisper-cool `#F4F7FE` accent tint panel.
...
### Elevation & motion
- **Slate-tinted shadows** — `rgba(12,19,34, .06–.16)`, never harsh black. Cards rest at `shadow-xs/sm`, lift to `sm/md` on hover.
- **Primary buttons get a blue glow** (`--rc-shadow-accent`) on hover; press removes the glow and darkens — **no scale transform**.
- **Easing `cubic-bezier(0.2,0.7,0.2,1)`**, durations 120 / 200 / 320ms. Restrained: fades and soft elevation, no bounce, no looping decorative animation. A subtle radial blue glow sits behind dark hero/CTA sections.
...
| Brand accent | **Confirmed** | Bright blue `#1E66FF`, set by the design owner. Legacy heritage orange (`#FF5A1F`) is retained as an alternate in the prototype's Tweaks panel only. |
